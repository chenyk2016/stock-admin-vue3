...
_.arrWithout = function (master, slave) {
  ...
  for (let i = 0; i < slave.length; i++) {
    const index = master.indexOf(slave[i])
...
_.arrUnique = function (arr) {
  const temp = {}; const r = []; const len = arr.length; let val; let type
  for (let i = 0; i < len; i++) {
    val = arr[i]
    type = typeof val
    if (!temp[val]) {
...
_.deepAssign = function (target, ...sources) {
  const targetIsObj = _.isObject(target)
...
_.strHex = function (str, key) {
  let hex = ''
  const len = { str: str.length, key: key.length }
